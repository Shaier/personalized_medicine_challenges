...
os.system(f'mkdir -p {repo_root}/data/medqa_usmle/statement')

for fname in ["train", "dev", "test"]:
    with open(f"{repo_root}/data/dataset_files/{fname}.jsonl") as f: # ADDED A FOLDER CALLED DATASET_FILES. PUT THE DEV FILE (copy it from the qagnn_data folder one at a time) IN IT WITH THE TRAIN,TEST SAMPLE FILES 
        lines = f.readlines()
    examples = []
    ...
    with open(f"{repo_root}/data/medqa_usmle/statement/{fname}.statement.jsonl", 'w') as fout:
        for dic in examples:
